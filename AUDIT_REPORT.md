...
- **Cart total omits tax → IMPLEMENTED (tax-aware cart).**
  The backend now returns the org `tax_rate` on `/auth/login` + `/auth/me`; it flows into `AuthState` and is injected into the cart. The cart computes a tax-inclusive total matching the backend exactly (`taxableAmount = subtotal − discount`, `taxAmount = round(taxableAmount × taxRate)`, `total = taxable + tax`). Tender/split validation and change already key off `cart.total`, so they're now correct; the checkout draft/optimistic orders carry `cart.taxAmount`; the cart footer + checkout summary show a Tax line when tax > 0. Backward-compatible: `taxRate = 0.0` (every current org) ⇒ `total == subtotal − discount`, unchanged. (`flutter test` 325/0.)
