...
class PaymentMethodNotifier extends Notifier<PaymentMethodState> {
  ...
  Future<void> load(String orgId, {bool force = false}) async {
    ...
    // alone used to leak the previous branch's methods on network failure.)
    if (state.loadedOrgId != orgId || state.items.isEmpty) {
      ...
      List<PaymentMethod> cachedItems;
      try {
        cachedItems = cachedRaw.map(PaymentMethod.fromJson).toList();
      } catch (_) {
        cachedItems = const [];
      }
