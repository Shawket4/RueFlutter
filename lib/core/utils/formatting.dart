...
// Render instants in the branch's configured timezone (AppTz), never the
// device's local zone, so times are correct on a mis-set till.
String timeShort(DateTime dt) => DateFormat('hh:mm a').format(AppTz.local(dt));
String dateShort(DateTime dt) => DateFormat('MMM d').format(AppTz.local(dt));
String dateTime(DateTime dt)  => DateFormat('MMM d, hh:mm a').format(AppTz.local(dt));
