import 'package:test/test.dart';
import 'package:sufrix_api/sufrix_api.dart';

// tests for MenuItemWithCosts
void main() {
  final MenuItemWithCosts? instance = /* MenuItemWithCosts(...) */ null;
  // TODO add properties to the entity

  group(MenuItemWithCosts, () {
    // int basePrice
    test('to test the property `basePrice`', () async {
      // TODO
    });

    // String categoryId
    test('to test the property `categoryId`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // String defaultMilkAddonId
    test('to test the property `defaultMilkAddonId`', () async {
      // TODO
    });

    // DateTime deletedAt
    test('to test the property `deletedAt`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // Object descriptionTranslations
    test('to test the property `descriptionTranslations`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String imageUrl
    test('to test the property `imageUrl`', () async {
      // TODO
    });

    // bool isActive
    test('to test the property `isActive`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // Object nameTranslations
    test('to test the property `nameTranslations`', () async {
      // TODO
    });

    // String orgId
    test('to test the property `orgId`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // List<SkuCost> skuCosts
    test('to test the property `skuCosts`', () async {
      // TODO
    });

  });
}
