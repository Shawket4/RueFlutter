import 'package:test/test.dart';
import 'package:sufrix_api/sufrix_api.dart';

// tests for OrderItemFull
void main() {
  final OrderItemFull? instance = /* OrderItemFull(...) */ null;
  // TODO add properties to the entity

  group(OrderItemFull, () {
    // String bundleId
    test('to test the property `bundleId`', () async {
      // TODO
    });

    // int bundleUnitPrice
    test('to test the property `bundleUnitPrice`', () async {
      // TODO
    });

    // True when any cost component could not be resolved.
    // bool costMissing
    test('to test the property `costMissing`', () async {
      // TODO
    });

    // Object deductionsSnapshot
    test('to test the property `deductionsSnapshot`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String itemName
    test('to test the property `itemName`', () async {
      // TODO
    });

    // Full line COGS in piastres (recipe + addons + optionals + components). `null` ⟺ unknown.
    // int lineCost
    test('to test the property `lineCost`', () async {
      // TODO
    });

    // int lineTotal
    test('to test the property `lineTotal`', () async {
      // TODO
    });

    // String menuItemId
    test('to test the property `menuItemId`', () async {
      // TODO
    });

    // Object nameTranslations
    test('to test the property `nameTranslations`', () async {
      // TODO
    });

    // String notes
    test('to test the property `notes`', () async {
      // TODO
    });

    // String orderId
    test('to test the property `orderId`', () async {
      // TODO
    });

    // int quantity
    test('to test the property `quantity`', () async {
      // TODO
    });

    // String sizeLabel
    test('to test the property `sizeLabel`', () async {
      // TODO
    });

    // Recipe-only cost per unit in piastres (incl. swaps). `null` ⟺ unknown or bundle line.
    // int unitCost
    test('to test the property `unitCost`', () async {
      // TODO
    });

    // int unitPrice
    test('to test the property `unitPrice`', () async {
      // TODO
    });

    // List<OrderItemAddon> addons
    test('to test the property `addons`', () async {
      // TODO
    });

    // List<OrderBundleComponentFull> bundleComponents
    test('to test the property `bundleComponents`', () async {
      // TODO
    });

    // List<OrderItemOptional> optionals
    test('to test the property `optionals`', () async {
      // TODO
    });

  });
}
